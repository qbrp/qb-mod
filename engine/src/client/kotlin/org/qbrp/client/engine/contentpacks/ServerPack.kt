...
import org.qbrp.main.engine.assets.resourcepack.versioning.PackManifest
import java.io.File
...
class ServerPack(val path: File): Asset() {
    ...
    val resourcePack = path.resolve(PACK_NAME)
    ...
    val name = path.name
    val manifest: PackManifest = Core.get<AssetsAPI>().getOrLoad(
        JsonFileReference(SimpleKey(path.resolve("manifest")), PackManifest::class)
    )
    val version
        get() = manifest.version

    fun getPackProfileAndCopyTo(resourcePacksPath: File = FileSystem.MINECRAFT_RESOURCEPACKS): ResourcePackProfile {
        resourcePack.copyRecursively(resourcePacksPath.resolve(PACK_NAME), true)
